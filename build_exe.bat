...
taskkill /f /im Calculator.exe 2>nul
timeout /t 1 /nobreak >nul
pip install customtkinter pyinstaller pillow
python create_icon.py
python create_ico.py
pyinstaller --clean --onefile --windowed --name Calculator --icon icon.ico --add-data "icon.ico;." --add-data "icon.png;." calculator.py
pyinstaller --clean --onefile --windowed --name update --icon icon.ico update_checker.py
echo.
...
echo.
echo Fertig! EXE: dist\Calculator.exe
if exist installer\ echo Installer: installer\Taschenrechner_Setup.exe
pause
